...
function App() {
    ...
    const [isFazalPlaying, setIsFazalPlaying] = useState(false);
    const audioRefs = useRef<{ [key: number]: HTMLAudioElement }>({});
    const fazalAudioRef = useRef<HTMLAudioElement | null>(null);
    ...
    const handleFazalPlay = () => {
        ...
        if (isFazalPlaying) {
            // Pause Fazal's audio
            if (fazalAudioRef.current) {
                fazalAudioRef.current.pause();
                fazalAudioRef.current.currentTime = 0;
            }
            setIsFazalPlaying(false);
        } else {
            // Play Fazal's audio
            if (!fazalAudioRef.current) {
                const audio = new Audio('/voices/fazal.mp3');
                audio.addEventListener('ended', () => setIsFazalPlaying(false));
                audio.addEventListener('error', () => {
                    console.log('Fazal message audio not found, using demo mode');
                    // Demo mode - auto stop after 5 seconds
                    setTimeout(() => setIsFazalPlaying(false), 5000);
                });
                fazalAudioRef.current = audio;
            }

            // Try to play the audio
            fazalAudioRef.current.play().catch(() => {
                console.log('Fazal message audio not found, using demo mode');
                // Demo mode - auto stop after 5 seconds
                setTimeout(() => setIsFazalPlaying(false), 5000);
            });
            setIsFazalPlaying(true);
        }
    };
                ...
                <div className="text-center z-10">
                    ...
                    >
                        ...
                        <div className="flex items-center justify-center space-x-4">
                            <WaveformAnimation isPlaying={isFazalPlaying} />

                            <motion.button
                                whileHover={{ scale: 1.1 }}
                                whileTap={{ scale: 0.95 }}
                                onClick={handleFazalPlay}
                                className="bg-gradient-to-r from-pink-500 to-purple-600 text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
                            >
                                {isFazalPlaying ? <Pause size={20} /> : <Play size={20} />}
                            </motion.button>
                        </div>

                        {isFazalPlaying && (
                            <p className="text-white/80 text-sm mt-2 italic">Playing message...</p>
                        )}
                    </motion.div>
                </div>
